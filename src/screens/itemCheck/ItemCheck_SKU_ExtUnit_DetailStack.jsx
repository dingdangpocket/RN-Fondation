...
const ItemCheck_SKU_ExtUnit_DetailStack = ({ route }) => {
  const {
    skuDetail,
    inboundApplyNoteDetailId,
    receivingNoteNo,
    receivingCheckNumber,
  } = route?.params;
  ...
  }, [units, units[0], units[1], PurchaseUnit]);

  const onSubmit = async () => {
    const fet = async () => {
      if (!ctxState || !timestampId || !receivingNoteNo || !units) {
        // console.log(ctxState, timestampId, receivingNoteNo, units);
    ...
    if (skuDetail?.unitConvert) {
      // console.log(
      //   "PMS固定转换值",
      //   skuDetail?.unitConvert,
      //   "计算转换值",
      //   convertVal,
      //   "差值",
      //   skuDetail?.unitConvert - convertVal
      // );
      // return;
      ...
      >
        ...
        >
          ...
          <Content
            color={"#222222"}
            fontSize={14}
            value={`${skuDetail?.stockComingNum}${skuDetail?.stockUnit}` ?? "-"}
          />
        </View>
        {/* 采购单位到货数量 */}
        <View style={{ ...ST }}>
...
export default ItemCheck_SKU_ExtUnit_DetailStack;
