...
const ItemCheck_Sync_WR_DetailStack = ({ route }) => {
  ...
  const onRightFun = () => {
    navigation.navigate("ItemCheck_Sync_Done_Stack");
  };
  //onActivehook
  ...
  const getPolist = async () => {
    ...
    const response = await fetchData({
      path: `/inbound/receiving/getInboundSupplier`,
      method: "POST",
    ...
    if (response.code == 200) {
      ...
      setInboundApplyNoteId(response?.data?.inboundApplyNoteId);
      setReceivingNoteId(response?.data?.receivingNoteId);
  ...
  //200ms后未输入值、获取扫描值,存储到resultDone缓存;
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (resultDone) {
      ...
      if (!item) {
        ...
        setLoading(false);
        if (!ctxState?.receivingNoteNo || !inboundApplyNoteId) {
        ...
        item?.extUnit == ""
          ? navigation.navigate("ItemCheck_SKU_DetailStack", {
              ...
              originalNoteNo: ctxState?.receivingNoteNo,
  ...
  const handleSubmit = () => {
    ...
    if (scanResult) {
      const item = skuList.find((x) => {
        return String(x.skuId) == String(code);
      });
      if (!item) {
        ToastAndroid.show("未匹配到项次,请重新扫描或输入", ToastAndroid.SHORT);
        setScanResult("");
  ...
  return (
    ...
    >
      <NoTabHeadBar
        titleA={ctxState?.receivingNoteNo}
        icon={
...
export default ItemCheck_Sync_WR_DetailStack;
