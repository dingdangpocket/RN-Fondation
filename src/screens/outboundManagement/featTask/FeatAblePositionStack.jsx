...
//按单拣货列表;
//🚀查看可用库存增加SKUID参数;
const FeatAblePositionStack = ({ route }) => {
    ...
    const asyncWrapper = async () => {
      ...
      const response = await fetchData({
        ...
        bodyParams: {
          ...
          outboundApplyNoteDetailIds: last_itemDetail?.pickingNoteDetails?.map(
            (item) => {
              return item?.outboundApplyNoteDetailId;
            }
          ),
        },
      });
  ...
  useEffect(() => {
    ...
    if (!res) {
      ...
      navigation.navigate("FeatTaskDetailStack", {
        ...
        curSetItem: res,
        fromPage: "FeatAblePositionStack",
        pickingStorageBinId: res.pickingStorageBinId,
      });
    }
  }, [resultDone]);
    ...
    <View>
      ...
      >
        ...
        {goodsSet?.length == 0 && !loading && (
          ...
          >
            <Text style={{ color: "rgb(200,200,200)" }}>空</Text>
          </View>
        )}
                ...
                <GoodsPositionDetailCard
                  ...
                  item1_left={
                    <Text style={{ fontSize: 18, color: "#006DCF" }}>
                      容器编码:{item?.containerCode}
                    </Text>
                  }
                  item2_left={
                    <Text style={{ fontSize: 15 }}>
                      货位编码:{item?.pickingStorageBinCode}
                    </Text>
                  }
                  item3_left={<Text style={{ fontSize: 15 }}>可用库存</Text>}
                  item3_right={
...
export default FeatAblePositionStack;
