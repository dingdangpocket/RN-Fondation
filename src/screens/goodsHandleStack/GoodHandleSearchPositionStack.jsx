...
const GoodHandleSearchPositionStack = () => {
  ...
  useEffect(() => {
    ...
    const getPosition = async () => {
      ...
      if (response.code == 200) {
        ...
        ToastAndroid.show(`获取成功${response?.msg}`, ToastAndroid.SHORT);
      } else {
          ...
          data.forEach((x) => {
            if (x.storageBins.find((k) => k == response.data.storageBinCode)) {
              navigation.navigate("GoodHandleTaskDetail", {
                ...
                fromPage: "GoodHandleSearchPositionStack",
              });
            } else {
              setAlertText(
              ...
              >
                <TouchableNativeFeedback onPress={() => onPress(item)}>
                  ...
                  >
                    ...
                    >
                      <Text style={{ fontSize: 20 }}>{item.row}</Text>
                      <View
                        style={{
                          height: rpx2dp(45, false),
                          alignItems: "center",
...
export default GoodHandleSearchPositionStack;
